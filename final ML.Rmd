...
```{r}
linear_fit = glm(log_area~month_dec + temp, data = newdata[-test,])
...
set.seed(123)
test = sample(nrow(fires_lm), floor(nrow(fires_lm)*.2))
logit_fit <- glm(large_fire ~ ., data = fires_lm[-test, ], family = "binomial")
summary(logit_fit)

#10 CV validation for the training set 
cv.glm(fires_lm[-test,], logit_fit, K = 10)$delta[1]

# getting MSE for the test
pred = predict(logit_fit, fires_lm[test,])
mean((pred - fires_lm[test, "large_fire"])**2)
```

```{r}
.88 / (max(fires_lm$log_area) - min(fires_lm$log_area))

table(fires$month)
```


Splitting the data and seeing what happens 
```{r}
log_fires = fires
log_fires$log_area = log(log_fires$area +1)
fires = log_fires[, -c(13)]
fires_with = fires[fires$log_area > 0, ]
fires_without = fires[fires$log_area == 0, ]
```


```{r}
barplot(table(fires_with$month), ylab = "Frequency", main = "Frequency of Forest Fires per Month with damage")
barplot(table(fires_without$month), ylab = "Frequency", main = "FRequency of Forest Fires per Month")
```

```{r}
hist(fires_with$rain,  main = "with damage")
hist(fires_without$rain, main = "without")
```
```{r}
hist(fires_with$temp,  main = "with damage")
hist(fires_without$temp, main = "without")

mean(fires_with$temp)
mean(fires_without$temp)
```
```{r}
barplot(table(fires_with$X))
barplot(table(fires_without$X))

table(fires_with$X)
table(fires_without$X)
```
```{r}
barplot(table(fires_with$Y))
barplot(table(fires_without$Y))

table(fires_with$Y, fires_with$X)
table(fires_without$Y, fires_without$Y)
```
```{r}
table(fires_with[fires_with$Y == 4, "X"])
```
